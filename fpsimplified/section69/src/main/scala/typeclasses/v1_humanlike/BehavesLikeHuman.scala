...
//Step1: define a behaviour in a trait that takes a generic type
trait BehavesLikeHuman[A] {
	def speak(a: A): Unit
	def eatHumanFood(a: A): Unit
}
//Step 2: create instances for the types we care about(Dog, int this case)
...
object BehavesLikeHuman {
	def speak[A](a: A)
		(implicit behavesLikeHumanInstance: BehavesLikeHuman[A]): Unit = {
